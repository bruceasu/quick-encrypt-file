...
public class FileMoveCmd implements Command {
...

    {
        ...
        description.put("-v", "verbose");
        description.put("[arguments]", "The input files or directories."
                + " The last is the target. if move many files, the targets should be a directory.");
    }
    ...
    private void moveFile(Path path, Path outputPath) throws IOException {
        if (verbose) System.out.printf("Moving %s to %s.%n", path, outputPath);
...
//        Files.move(path, outputPath, StandardCopyOption.REPLACE_EXISTING);
    ...
    public static void main(String[] args) throws Exception {
        ...
        new FileMoveCmd().execute(new String[]{ "-v",
                "target\\classes", "target\\tmp"
        });
